...
'''
    ...
    def add(self, character, console):
        ...
        weapon_roll = Utils.str2int(prompt('How many die in the weapons\'s base damage? > ', 
                validator=NumberValidator()
        ))

        die_types = ['2', '3', '4', '6', '8', '10', '12', '20']
        weapon_die = Utils.str2int(prompt('What kind of die? > ', 
                completer=WordCompleter(die_types), 
                validator=WordValidator(die_types)
        ))

        damage_types = Enums.weapon_damage_types + Enums.energy_damage_types
        weapon_type = prompt('what type of damage does the weapon do? > ', 
                completer=WordCompleter(damage_types), 
                validator=WordValidator(damage_types)
        )

        base_damage[weapon_type] = {'dice': f'{weapon_roll}d{weapon_die}'}
       
        more_damage_list = damage_types + ['no']
        more_damage = prompt('does the weapon do any additional damage? > ', 
                completer=WordCompleter(more_damage_list), 
                validator=WordValidator(more_damage_list)
        )
    ...
    def remove(self, character, console):
        ...
        weapon_to_remove = prompt('remove which weapon > ', 
