    ...
    def load_character(sheet_to_load):
        with open(sheet_to_load, 'r') as f:
            ...
            for name, attr in char_data['attributes'].items():
                character.attr_list.append(Attribute(name=name, total=attr['total'], bonus=attr['bonus'], base=attr['base'], item=0, mod=0
                                                     ))

            for name, sav in char_data['saves'].items():
                character.save_list.append(Save(name=name, total=sav['total'], ability=sav['ability'], bonus=sav['bonus'], base=sav['base'], item=0
                                                ))

            for name, skil in char_data['skills'].items():
                character.skill_list.append(Skill(name=name, total=skil['total'], ability=skil['ability'], rank=skil['rank'], bonus=skil['bonus'], class_=skil['class_'], item=0
                                                  ))
        ...
        try:
            ...
            for attr in character.attr_list:
                char_data['attributes'][attr.name] = {
                    'total': attr.total, 'bonus': attr.bonus, 'base': attr.base
                }

            char_data['saves'] = {}
            for save in character.save_list:
                char_data['saves'][save.name] = {
                    'total': save.total, 'ability': save.ability, 'bonus': save.bonus, 'base': save.base
                }

            char_data['skills'] = {}
            for skill in character.skill_list:
                char_data['skills'][skill.name] = {
                    'total': skill.total, 'ability': skill.ability, 'rank': skill.rank, 'bonus': skill.bonus, 'class_': skill.class_
                }
            ...
            if character.spell_list:
                for spell in character.spell_list:
                    char_data['spells'][str(spell.level)] = {
                        'save': spell.save, 'slots': spell.slots, 'remaining': spell.remaining, 'base': spell.base
                    }

            char_data['items'] = {}
            for item in character.item_list:
                char_data['items'][item.name] = {
                    'equipped': item.equipped, 'slot': item.slot, 'ac': item.ac, 'bonus': item.bonus
                }

            char_data['weapons'] = {}
            for weapon in character.weapon_list:
                char_data['weapons'][weapon.name] = {
                    'damage': weapon.damage, 'bonus_damage': weapon.bonus_damage
                }

            char_data['attacks'] = {}
            for attack in character.attack_list:
                char_data['attacks'][attack.name] = {
                    'weapon': attack.weapon, 'attack_mod': attack.attack_mod, 'bonus_attack': attack.bonus_attack, 'damage_mod': attack.damage_mod, 'bonus_damage': attack.bonus_damage
                }

            char_data['feats'] = character.feat_list

            char_file = PurePath(f'{Config.sheets_path}', character.name.replace(
                ' ', '_'), f'CRIT{character.name}.toml'.replace(' ', '_'))
            really = True
            if os.path.isfile(char_file):
                really = prompt(
                    f'sheet for {character.name} exists, overwrite? > ')
            if not really:
                return
            else:
